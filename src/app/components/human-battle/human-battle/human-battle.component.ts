...
export class HumanBattleComponent implements OnInit, OnDestroy {
  ...
  public generateNewPlayers() {
    ...
    this.leftHuman$ = this.apiService.getHumanById(leftHumanId).pipe(takeUntil(this.destroyed$));
    this.rightHuman$ = this.apiService.getHumanById(rightHumanId).pipe(takeUntil(this.destroyed$));
